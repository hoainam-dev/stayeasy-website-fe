...
import { verifyForgotPassword } from "../../redux-tookit/actions/emailActions";
import { resetPassword } from "../../redux-tookit/actions/authActions";
import "./Loading.css";
...
export default function ForgotPassword({ state }) {
  ...
  function togglePasswordVisibility(index) {
    index===0?setIsPasswordVisible((prevState) => !prevState):
    setIsConfirmPasswordVisible((prevState) => !prevState)
  }

  // Send Email Code Area
  const [code, setCode] = useState(); //code random
  const [confirmCode, setConfirmCode] = useState(""); //code input

  const [codeError, setCodeError] = useState(); //message code error
  const [codeSuccess, setCodeSuccess] = useState(); //message code success

  // state countdown -> countdown time to resend code
  const [countdown, setCountdown] = useState(60);

  // set state message
  const setMessage = ( successMessage, errorMessage, codeError, isSendCode, isVerify) =>{
      setCodeSuccess(successMessage);
  ...
  const dataVerifyForgotPassword = {
    email: email,
    setCode: setCode,
  ...
  const handleSendCode = () => {
      if (email==="" ||!email) {
        ...
        setErrorMessage("Email không hợp lệ!");
          ...
          if (!isSendCode) {
              setIsSending(true);
              dispatch(verifyForgotPassword(dataVerifyForgotPassword));
  ...
  const handleReSendCode = () =>{
      if(!isSendCode){
        ...
        return () => clearInterval(interval);
          ...
          setErrorMessage("Vui lòng thử lại sau 60 giây.");
      }
  }

  // method validate email input and send email from authAction
  const handleCheckCode = () => {
      if (code!=confirmCode.toUpperCase()) {
          setCodeSuccess();
          setCodeError("Mã xác minh không hợp lệ!");
      } else {
        setIsVerify(true);// đã xác thực thành công
        setIsSecondForm(true);// chuyển qua trang thứ 2
        setCodeError();// đặt lại message error của code
      }
    }
    ...
    <>
      ...
      {isSecondForm?(
          <>
              {/* password */}
              <div className="mb-6">
                  <label htmlFor="password" className="block text-gray-800 font-medium mb-2">Mật khẩu mới<span className="text-red-500"> (*)</span></label>
              ...
              <div className="flex items-center justify-between">
                <button onClick={() => { isVerify?handleResetPassword():console.log("Ban chua xac thuc"); }} type="button"
          ...
          <>
              ...
              {!codeError == "" ? (
                  <div class="bg-red-100 border border-red-400 text-red-700 px-4 py-3 mb-3 rounded relative" role="alert">
                      <span class="block sm:inline">{codeError}</span>
                  </div>
              ) : ("")}
              ...
              {!codeSuccess == "" ? (
                  <div class="bg-green-100 border border-green-400 text-green-700 px-4 py-3 mb-3 rounded relative" role="alert">
                      <span class="block sm:inline">{codeSuccess}</span>
                  </div>
              ) : ("")}
              ...
              <div className="flex items-center mb-6 w-full">
                  <div className="w-[45%]">
                      ...
                      className="border rounded-2xl text-gray-700 px-[1.4rem] py-3 my-2 w-full"
                      value={confirmCode?confirmCode.toUpperCase():confirmCode}
              ...
              <div className="flex items-center justify-between">
                <button className="bg-[#da0964] hover:bg-[#ff002bda] transition duration-1000 text-white font-bold py-3 px-4 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 w-full"
                  onClick={() => {isVerify?handleCheckCode():handleSendCode()}} 
